...
int GetMask(double lat, double lon, struct dem dem[])
{
...
int PutMask(double lat, double lon, int value, struct dem dem[])
{
	/* Lines, text, markings, and coverage areas are stored in a
	   mask that is combined with topology data when topographic
         ...
         strmode, errnum);
			...
			if (LR.erp!=0.0)
			{
				if (dbm)
				{
					...
					PutSignal(path.lat[y],path.lon[y],(unsigned char)ifs,dem);
				}
			...
			PutMask(path.lat[y],path.lon[y],
                (GetMask(path.lat[y],path.lon[y])&7)+(mask_value<<3), 
                dem);
		}
	}
}
